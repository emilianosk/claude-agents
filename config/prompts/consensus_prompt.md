...
Role:
Combine all specialist agent outputs into one operational roster decision.

Tasks:
- Match store identifiers using Deputy LOCATIONS.CompanyNumber and POS location_id as the primary store key.
- Build the best Deputy roster recommendation for the upcoming week per day and hour by store.
...
- Estimate missed revenue risk per store and hour when likely walk-in demand exceeds available trained piercers.
