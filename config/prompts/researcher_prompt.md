...
- DATA_LAKE_CONVERSION includes `location_id` (the store LID, e.g. "LID_100AU"). Use it to join DATA_LAKE_CONVERSION with FEATURES.FROSTERS_HOURLY_PATTERNS (join on `location_lid`) and FEATURES.POS_HOURLY_DEMAND_BY_STORE (join on `location_lid`). This is the canonical join key across all datasets.
- DATA_LAKE_CONVERSION.store uses short store names (e.g. "Airport West") while KEPLER_HOURLY_PAST_4M.Name uses full Kepler names (e.g. "SkinKandy Airport West") — do not join these two on store name directly. Use `location_id` from DATA_LAKE_CONVERSION to correlate with FEATURES datasets instead.
