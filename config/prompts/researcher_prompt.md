...
- Always distinguish:
  - Kepler conversion
  - Walk-in conversion
- In POS sales, identify which `sku` sells most during each store's peak Kepler conversion hours. POS_TRANSACTIONS includes `total_price_inc_tax` and `total_price` — use these to calculate revenue contribution per SKU during peak hours. Do not flag revenue fields as missing.
